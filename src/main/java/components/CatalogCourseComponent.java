...
public class CatalogCourseComponent extends AnyComponentAbs<CatalogCourseComponent> {
  ...
  @Inject
  public CatalogCourseComponent(DIScope diScope) {
    super(diScope);
  ...
  public CourseDetailPage clickCourseByName(String courseName) {
    ...
    return new CourseDetailPage(diScope);
  }
  ...
  public void assertCourseName(String courseName) {
    if (diScope.getWaiter().waitForElementVisible($(header))) {
      Assert.assertEquals($(header).getText(), courseName);
    } else {
      Assert.fail("Element not visible page");
    }
  }
